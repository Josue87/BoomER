...
class Payload():
    ...
    def get_info_metasploit(self):
        ...
        return f"Compatible with Metasploit: {self.info_metasploit}"
        ...
        if not self.single:
            ...
            if "encode" in self.options and self.options["encode"][1]:
                ...
                sh_aux = (
                    self.shellcode % (p, h)
                    if self.arq == "x64"
                    else self.shellcode % (h, p)
                )
        elif "encode" in self.options and self.options["encode"][1]:
            if self.arq == "x64":
    ...
    def put(self, key, value):
        try:
            ...
            custom_print.info(f"{key} => {value}")
        except Exception:
            custom_print.error(f"Wrong option: {key}")
